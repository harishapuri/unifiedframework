...
function resetFlowDiagram() {
  document.querySelectorAll(".flow-node").forEach((el) => el.classList.remove("active", "done"));
  document.getElementById("flowIngest").textContent = "Security scan + live traffic";
  document.getElementById("flowCrc").textContent = "Did the rules pass?";
  document.getElementById("flowZeroguard").textContent = "Is the setup locked down?";
  document.getElementById("flowInfraagent").textContent = "Will it stay healthy?";
  document.getElementById("flowGate").textContent = "Go / wait / stop";
  document.getElementById("flowAudit").textContent = "Written down and sealed";
}
...
function updateGateBanner(decision) {
  ...
  document.getElementById("gateAction").textContent = word;
  document.getElementById("gateReasons").textContent = [plain, decision.reasons.join("; ")].filter(Boolean).join(" ");
}

function updateAuditPanel(auditDetail, decision, shadow) {
  document.getElementById("chainOk").textContent = auditDetail.chain_ok ? "intact" : "broken";
  document.getElementById("chainOk").style.color = auditDetail.chain_ok ? "var(--ok)" : "var(--bad)";
  const actionPart = decision ? `  ${decision.action}` : "";
  document.getElementById("auditEntry").textContent =
    `Record #${auditDetail.index} sealed` + actionPart;
}

function renderFinal(data) {
  const { crc, zeroguard, infraagent, governance } = data;
  updateCrcPanel(crc);
  updateZgPanel(zeroguard);
  updateIaPanel(infraagent);
...
function handleEvent(ev) {
  switch (ev.stage) {
    case "scenario_start":
      ...
      document.getElementById("scenarioBlurb").textContent = ev.detail.blurb;
      logEvent(ev, `▶ ${ev.detail.blurb}`);
      break;
    case "ingest":
      setActiveNode("ingest");
      document.getElementById("flowIngest").textContent = "reading the scan…";
      logEvent(ev, "Reading the security scan and live traffic");
      break;
    case "ingested":
      document.getElementById("flowIngest").textContent = `${ev.detail.n_passed} passed / ${ev.detail.n_failed} failed`;
      logEvent(ev, `Scan done: ${ev.detail.n_passed} passed, ${ev.detail.n_failed} failed`);
      break;
    case "crc":
      setActiveNode("crc");
      document.getElementById("flowCrc").textContent = ev.detail.residual_high ? "leftover risk — high" : "rules look fine";
      updateCrcPanel(ev.detail);
      logEvent(ev, `Code checks: ${Math.round(ev.detail.eta * 100)}% of rules passed`);
      break;
    case "zeroguard":
      setActiveNode("zeroguard");
      document.getElementById("flowZeroguard").textContent = `trust ${Math.round(ev.detail.psi * 100)}%`;
    ...
    case "infraagent":
      ...
      document.getElementById("flowInfraagent").textContent = `trouble in 1h ${Math.round(ev.detail.phi_1h * 100)}%`;
    ...
    case "gate": {
      ...
      document.getElementById("flowGate").textContent = word;
      document.querySelector('.flow-node[data-stage="gate"]').classList.add(gateClass(ev.detail.dsa));
    ...
    case "audit":
      setActiveNode("audit");
      document.getElementById("flowAudit").textContent = ev.detail.chain_ok ? "signed and intact" : "log is broken";
...
function startStream(story) {
  ...
  const es = new EventSource(`/api/stream?story=${encodeURIComponent(story)}`);
  currentSource = es;
